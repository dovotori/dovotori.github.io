...
export default class {
  ...
  updateLinear = (lockX, lockY, lockZ) => {
    ...
    if(!lockX){ this.position.x += this.vitesse.x; }
      if(!lockY){ this.position.y += this.vitesse.y; }
      if(!lockZ){ this.position.z += this.vitesse.z; }
      ...
      this.vitesse = this.vitesse.multiplierValeur(DAMPING);
  }
  ...
  rebond = () => {
    if(this.position.x < MIN_X){this.vitesse.x *= -1;}
    if(this.position.x > MAX_X){this.vitesse.x *= -1;}
    if(this.position.y < MIN_Y){this.vitesse.y *= -1;}
    if(this.position.y > MAX_Y){this.vitesse.y *= -1;}
    if(this.position.z < MIN_Z){ this.vitesse.z *= -1; }
    if(this.position.z > MAX_Z){ this.vitesse.z *= -1; }
  }

  respawnBordures = () => {
    if (this.position.x < -SIZE){ this.position.x = MAX_X + SIZE; }
    if (this.position.y < -SIZE){ this.position.y = MAX_Y + SIZE; }
    if (this.position.z < -SIZE){ this.position.z = MAX_Z + SIZE; }

    if (this.position.x > MAX_X + SIZE){ this.position.x = -SIZE; }
    if (this.position.y > MAX_Y + SIZE){ this.position.y = -SIZE; }
    if (this.position.z > MAX_Z + SIZE){ this.position.z = -SIZE; }
  }

  setPosition = (x, y, z) => { this.position.set(x, y, z); }

  setSpeed = (x, y, z) => { this.vitesse.set(x, y, z); }

  getPosition = () => { return this.position; }

  getSpeed = () => { return this.vitesse; }
}