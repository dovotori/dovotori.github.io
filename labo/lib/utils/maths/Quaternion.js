...
class Quaternion {
  ...
  static fromUnitVectors(a, b) {
    // a and b are Vec3 (assumed normalized or close). Returns a new Quaternion.
    const dot = a.dot(b);
    ...
    // opposite direction -> rotate 180deg around any perpendicular axis
    if (dot < -0.999999) {
    ...
    const q = new Quaternion(cross.getX(), cross.getY(), cross.getZ(), 1 + dot);
    return q.normalize();
  }
}
