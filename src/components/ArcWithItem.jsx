...
const ArcWithItem = ({
  x, y, radius, startAngle, className, name, image, depth, strokeWidth, children, onClick, angle, margin, noHoverAnim
}) => {
  ...
  const visible = useMemo(() => !(startAngle === 360 && angle === 0), [startAngle, angle]);

  return (
    <Wrap className={className} onClick={onClick} noHoverAnim={noHoverAnim}>
      <StyledArc x={x} y={y} radius={radius} startAngle={startAngle} endAngle={endAngle} depth={depth} visible={visible} />
      {visible && depth <= 2
      && (
      <G>
        {image ? (
