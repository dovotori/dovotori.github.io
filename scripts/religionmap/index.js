...
const SAVE_HTML_FILE = path.resolve(
  __dirname,
  "../../public/html/religionmap.html",
);
...
const generateSvg = async (showBrowser) => {
  ...
  const countriesDataFile = path.resolve(
    __dirname,
    "../common/worldCountries.csv",
  );
  ...
  const html = await utils.readFile(
    path.resolve(__dirname, "./createSvg.html"),
    "utf8",
  );
  const css = await utils.readFile(
    path.resolve(__dirname, "./createSvg.css"),
    "utf8",
  );
  ...
  const svgData = await page.$eval(
    "svg#worldmap",
    (domItem) => domItem.outerHTML,
  );
...
const generateHtml = async (showBrowser) => {
  ...
  const infos = await utils.readFile(
    path.resolve(__dirname, "./infos.html"),
    "utf8",
  );
  return `
<style>${css}</style>
...
const main = async () => {
  ...
  const showBrowser =
    myArgs.indexOf("-s") !== -1 || myArgs.indexOf("--show") !== -1;
