  ...
  const setup = () => {
    ...
    blasphemes.forEach((blasphem) => {
      const country = countries.filter(
        (c) => c["ISO3166-1-Alpha-3"] === blasphem.iso,
      );
      if (country && country[0]) {
        const path = d3.select(
          `g.map path[data-iso="${country[0]["ISO3166-1-numeric"]}"]`,
        );
        if (path) {
          ...
          if (node && blasphem.mort === "oui") {
            ...
            groupDead
              ...
              .attr(
                "transform",
                `translate(${x + width / 2}, ${y + height / 2}) scale(0.15)`,
              );
          }
        }
      }
    ...
    document
      ...
      .forEach((countryPath) => {
        ...
        const countryData = countries.filter(
          (d) => d["ISO3166-1-numeric"] === isoAlpha,
        );
        if (countryData && countryData[0]) {
          const religion = religions.filter(
            (r) => r.name === countryData[0]["ISO3166-1-Alpha-3"],
          );
