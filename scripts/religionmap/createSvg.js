...
const main = () => {
  ...
  const setup = () => {
    ...
    const filteredGeometries = geojson.objects.countries.geometries.filter(country => country.id !== "010");
      ...
      if (country && country[0]) {
        ...
        if (path) {
          ...
          if (node && blasphem.mort === "oui") {
            ...
            groupDead.append('svg').attr("class", "deadPicto").append('path').attr("d", "M24.869 -17.798 L17.798 -24.869 L0 -7.071 L-17.797 -24.869 L-24.869 -17.798 L-7.071 0 L-24.869 17.798 L-17.798 24.869 L0 7.071 L17.798 24.869 L24.869 17.798 L7.071 0Z").attr("transform", `translate(${x + (width / 2)}, ${y + (height / 2)}) scale(0.15)`);
          }
        }
      }
    });

    document.querySelector(".map").querySelectorAll("path").forEach(countryPath => {
